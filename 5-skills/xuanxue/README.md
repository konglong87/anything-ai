---
title: "玄学技能包 Xuanxue Skills - 八字/风水/手相/奇门"
difficulty: beginner
roles: [all]
type: guide
duration: 30min
tools: [claude-code, codex, cursor, trae]
tags: [skills, 玄学, 八字, 风水, 手相, 奇门, 命理]
---

# 🔮 Xuanxue Skills - 中国传统玄学技能包

> **GitHub**: [konglong87/xuanxue-skills](https://github.com/konglong87/xuanxue-skills)
>
> **版本**: v0.2.0 | **许可证**: MIT
>
> **定位**: 给你的 AI Agent 增加八字、婚恋、事业财运、手相和奇门局盘判读能力

## 📖 简介

**Xuanxue Skills** 是一站式中国传统玄学技能包，覆盖算命、大运流年、风水、手相、婚姻情感、事业财运等能力。

它的设计理念与普通提示词不同：

- **不需要记技能名称**：不用背 `bazi`、`qimen` 之类的命令，直接用日常语言提问
- **不需要懂命理术语**：不知道"换日""起运""格局"是什么也能用
- **代码计算代替心算**：四柱、节气、真太阳时、大运流年由共享计算内核完成，不让模型凭印象算
- **仅供娱乐与研究**：不构成医疗、投资、职业或法律建议

## ✨ 当前能力

| 能力 | 状态 | 你需要知道的边界 |
|---|---|---|
| **八字综合** (bazi) | 已开放 | 代码排盘，并列展示换日、起运、旺衰、格局和喜用等流派差异 |
| **婚恋情感** (love-marriage) | 已开放 | 分析倾向和相处模式，不断言必婚、必离或他人的隐私事实 |
| **事业财运** (wealth-career) | 已开放 | 分析方向、节奏和风险，不保证收益，不替代投资或职业建议 |
| **手相** (palm) | 已开放 | 宿主 Agent 负责多模态观察（看图），代码负责验证观察结构；结论必须对应照片中的实际特征 |
| **奇门局盘** (qimen) | 已开放 | 只判读可信外部 APP 已起好的局盘，当前不自行起局 |
| **面相** (face-reading) | 规划中 | 因人脸隐私暂不开放 |
| **风水纳气** (fengshui-naqi) | 规划中 | 计算内核已有部分能力，完整用户技能尚未开放 |
| **占卜** (divination) | 规划中 | 尚未实现，不在当前安装清单中 |

## 🚀 快速开始（30 秒）

只要当前 Agent 能执行终端命令，就可以完成安装。

**第 1 步**：在 WorkBuddy、Codex、Claude Code、Cursor 或 Trae 里直接说：
...
- 把 `codex` 替换为 `claude-code`、`cursor`、`trae` 或 `workbuddy`
- `--scope user` 安装到个人环境；在目标项目目录中用 `--scope project` 安装到当前项目
...
- `core/` 不依赖 `skills`，五个宿主共享同一份计算内核和运行时
- 运行时**零外部依赖**；内联的 `lunar-javascript` 保留 MIT 许可证与来源
- 开发验证：`npm install && npm test -- --runInBand && node scripts/e2e-smoke.js`

### Agent 兼容状态

`verified` 表示已完成宿主发现和真实调用；`experimental` 表示安装器已验证，但宿主真机证据仍不完整。
...
- 本项目用于**中国传统术数文化研究与娱乐**，不构成医疗、投资、职业、法律或其他专业建议
...
- [AI算命师应用指南](../../roles/fortune-teller/README.md) - 了解算命排盘原理与提示词模板
- [工具矩阵](../../2-choose-tools/tool-matrix.md) - 算命娱乐工具选择参考