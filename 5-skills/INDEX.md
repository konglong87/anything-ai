---
title: "AI Skills 完整索引"
difficulty: beginner
roles: [all]
type: guide
duration: 30min
tools: []
tags: [skills, index, catalog]
---

# 📚 AI Skills 完整索引

> **快速查找你需要的Skills**

## 🗂️ 按分类索引

### 🔬 研究与分析类 (Research)
- [AI Research Skills](./research/ai-research-skills.md) - AI研究辅助，学术研究利器
- [Obsidian Skills](./research/obsidian-skills.md) - 知识管理，笔记整理神器

**适用场景**：学术研究、深度分析、知识管理、文献综述

**详细文档**：[研究类Skills目录](./research/README.md)

---

### 💻 开发相关类 (Development)
- [GStack](./development/gstack.md) - 技术栈管理，项目脚手架
- [Gstack (Claude Code Skills)](./development/gstack-claude-code.md) - YC创业方法论，Claude Code技能包，26个斜杠命令覆盖全流程
- [Impeccable](./impeccable-skill/README.md) - 零设计基础也能写出专业级前端页面，18个命令覆盖前端开发全流程

**适用场景**：
- GStack：项目创建、技术栈配置、快速开发
- Gstack (Claude Code)：0→1 MVP设计、产品审查、架构设计、安全审计
- Impeccable：前端页面优化、响应式设计、动效添加、代码审核、工程化

**详细文档**：[开发类Skills目录](./development/README.md)

---

### 📋 规划与组织类 (Planning)
- [Planning with Files](./planning/planning-with-files.md) - 文件规划，任务管理

**适用场景**：项目管理、文档整理、任务规划

**详细文档**：[规划类Skills目录](./planning/README.md)

---

### ⚡ 效率提升类 (Productivity)
- [Superpower Skills](./productivity/superpower-skills.md) - AI技能扩展，工作流优化，技能组合，工具集成
  - 技能扩展：丰富的技能库，易于安装和配置
  - 工具集成：支持多种API和服务，统一接口设计
  - 任务自动化：自动化重复任务，智能任务调度
  - 性能优化：高效执行引擎，资源管理，并行处理

**适用场景**：
- 任务自动化：自动化重复任务，工作流优化
- 技能扩展：为AI助手增加新能力
- 工具集成：集成第三方API和服务
- 性能提升：优化执行效率，资源管理

**详细文档**：[效率类Skills目录](./productivity/README.md)

---

### 🤖 Agent类
- [Awesome Agent Skills](./agent/awesome-agent-skills.md) - Agent技能集合
- [Skills Catalog](./agent/skills-catalog.md) - 完整Skills目录

**适用场景**：Agent开发、自动化任务、智能助手

**详细文档**：[Agent Skills目录](./agent/README.md)

---

### 📚 课程学习类 (Course)
- [技能包设计模式课程](./course/README.md) - 从理论到实践，掌握AI技能包的核心设计理念
  - 入口模式：统一调度中心
  - 模板方法模式：流程编排
  - 责任链模式：质量保障
  - 策略模式：灵活决策
  - 并行模式：效率最大化
  - 守护模式：安全边界
  - 最佳实践与反模式

**适用场景**：技能包开发、架构设计、AI工具链构建

**详细文档**：[课程索引](./course/README.md)

---

## 🎯 按使用场景索引

### 快速开发项目
1. [GStack](./development/gstack.md) - 创建项目脚手架
2. [Gstack (Claude Code Skills)](./development/gstack-claude-code.md) - YC创业方法论，产品设计和全流程开发
3. [Impeccable](./impeccable-skill/README.md) - 前端页面优化，专业级效果
4. [Planning with Files](./planning/planning-with-files.md) - 规划任务
5. [Superpower Skills](./productivity/superpower-skills.md) - 自动化工作流
6. [技能包设计模式课程](./course/README.md) - 掌握技能包设计理念
7. [Obsidian Skills](./research/obsidian-skills.md) - 管理文档

### 研究与写作
1. [AI Research Skills](./research/ai-research-skills.md) - 研究分析
2. [Obsidian Skills](./research/obsidian-skills.md) - 知识整理
3. [Superpower Skills](./productivity/superpower-skills.md) - 内容分发

### 自动化工作流
1. [Superpower Skills](./productivity/superpower-skills.md) - 自动化
2. [Planning with Files](./planning/planning-with-files.md) - 流程规划
3. [Agent Skills](./agent/awesome-agent-skills.md) - 智能助手

---

## 📊 按功能特性索引

### 知识管理
- [Obsidian Skills](./research/obsidian-skills.md) ⭐⭐⭐⭐⭐
- [AI Research Skills](./research/ai-research-skills.md) ⭐⭐⭐⭐

### 项目管理
- [Planning with Files](./planning/planning-with-files.md) ⭐⭐⭐⭐⭐
- [GStack](./development/gstack.md) ⭐⭐⭐⭐
- [技能包设计模式课程](./course/README.md) ⭐⭐⭐⭐⭐

### 自动化
- [Superpower Skills](./productivity/superpower-skills.md) ⭐⭐⭐⭐⭐
- [Agent Skills](./agent/awesome-agent-skills.md) ⭐⭐⭐⭐

### 研究分析
- [AI Research Skills](./research/ai-research-skills.md) ⭐⭐⭐⭐⭐
- [Obsidian Skills](./research/obsidian-skills.md) ⭐⭐⭐⭐

---

## 🔍 快速查找指南

### 我是程序员
→ 首选：[GStack](./development/gstack.md)
→ 搭配：[Planning with Files](./planning/planning-with-files.md)

### 我是研究者
→ 首选：[AI Research Skills](./research/ai-research-skills.md)
→ 搭配：[Obsidian Skills](./research/obsidian-skills.md)

### 我是内容创作者
→ 首选：[Obsidian Skills](./research/obsidian-skills.md)
→ 搭配：[Superpower Skills](./productivity/superpower-skills.md)

### 我是产品经理
→ 首选：[Planning with Files](./planning/planning-with-files.md)
→ 搭配：[AI Research Skills](./research/ai-research-skills.md)

---

## 📈 热门推荐 TOP 5

1. **Gstack (Claude Code Skills)** - YC创业方法论技能包 ⭐⭐⭐⭐⭐
   - YC总裁Garry Tan开源，26个斜杠命令
   - 2.5小时完成MVP全流程设计
   - 覆盖产品→架构→设计→测试→发布全流程
   - [查看详情](./development/gstack-claude-code.md)

2. **Superpower Skills** - AI技能扩展平台 ⭐⭐⭐⭐⭐
   - 丰富的技能库，易于安装配置
   - 工具集成，任务自动化，工作流优化
   - 支持Claude和ChatGPT
   - [查看详情](./productivity/superpower-skills.md)

3. **Obsidian Skills** - 知识管理神器 ⭐⭐⭐⭐⭐
   - 强大的笔记整理功能
   - 自动建立知识连接
   - [查看详情](./research/obsidian-skills.md)

4. **AI Research Skills** - 研究利器 ⭐⭐⭐⭐⭐
   - 快速文献分析
   - 深度研究支持
   - [查看详情](./research/ai-research-skills.md)

5. **GStack** - 项目脚手架工具 ⭐⭐⭐⭐
   - 一键创建项目，配置完整技术栈
   - 支持多种框架和语言
   - [查看详情](./development/gstack.md)

---

## 🔗 所有GitHub链接

- [planning-with-files](https://github.com/OthmanAdi/planning-with-files)
- [obsidian-skills](https://github.com/kepano/obsidian-skills)
- [AI-Research-SKILLs](https://github.com/Orchestra-Research/AI-Research-SKILLs)
- [awesome-agent-skills](https://github.com/VoltAgent/awesome-agent-skills)
- [superpower-skills](https://github.com/superpower/skills)
- [gstack](https://github.com/garrytan/gstack)

---

**快速开始**：选择一个适合你的Skill，立即开始使用！🚀